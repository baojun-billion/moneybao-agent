...
// 辅助函数：生成图片
async function generateImage(req, config) {
    const imageReq = {
    ...
    const response = await fetchWithRetry(`${config.apiEndpoint}/images/generations`, config, imageReq);
    const data = await response.json();
    if (!response.ok) {
        throw new Error(`图片生成失败: ${JSON.stringify(data)}`);
    }
    // 下载图片
    const images = await Promise.all(data.data.map(async (item) => {
...
// 辅助函数：生成视频
async function generateVideo(req, config) {
    const videoReq = {
        model: config.videoModel,
        input: {
            prompt: req.prompt,
    ...
    const response = await fetchWithRetry(`${config.apiEndpoint}/video/generations`, config, videoReq);
    const data = await response.json();
    if (!response.ok) {
    ...
    while (attempts < maxAttempts) {
        ...
        const statusResponse = await fetchWithRetry(`${config.apiEndpoint}/video/generations/${requestId}`, config, {});
...
// 辅助函数：生成文本
async function generateText(req, config) {
    const response = await fetchWithRetry(`${config.apiEndpoint}/chat/completions`, config, req);
    const data = await response.json();
    if (!response.ok) {
...
// 辅助函数：带重试的 fetch
async function fetchWithRetry(url, config, body) {
    let lastError = null;
